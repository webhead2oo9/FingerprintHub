    ...
    with pool.connection() as conn:
        with conn.transaction():
            ...
            if existing is not None:
                ...
                row = conn.execute(
                    ...
                    (
                        ...
                        category, action, source_guild_id, enc_reason,
            ...
            row = conn.execute(
                ...
                (
                    phash_hex, algorithm, algorithm_version, normalization_version,
                    category, action, consumer_id, source_guild_id, enc_reason,
    ...
    with pool.connection() as conn:
        with conn.transaction():
            ...
            conn.execute(
                ...
                VALUES (%s, %s, %s, %s, %s)
                """,
                (fingerprint_id, consumer_id, guild_id, distance, now_ms),
            )
            row = conn.execute(
    ...
    with pool.connection() as conn:
        with conn.transaction():
            ...
            conn.execute(
                ...
                """,
                (fingerprint_id, consumer_id, reason, now_ms),
            )
            flag_count = int(
                conn.execute(
