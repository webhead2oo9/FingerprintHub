...
def _public_fp(row: Dict[str, Any]) -> Dict[str, Any]:
    """Sync-feed shape: no reason/source_url, no hit stats."""
    return dict(row)
...
def _detail_fp(request: web.Request, row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if _can_see_secrets(request, row):
...
async def sync_fingerprints(request: web.Request) -> web.Response:
    ...
    return web.json_response(
        {
            "fingerprints": [_public_fp(r) for r in rows],
