...
The runtime stack is Python 3.13+, aiohttp for HTTP, psycopg plus psycopg-pool
for Postgres, Alembic for schema history, and AES-GCM for application-level
encryption of `reason` / `source_url`. The default bind is localhost-only on
port 58751. Startup verifies that the database is reachable and exactly at the
repository's Alembic head; if either check fails it refuses to serve.
...
- `fingerprints`: the shared catalog. Key columns:
  ...
  - `reason` and `source_url`, encrypted at rest and redacted from non-owners.
...
- Tombstones: the feed includes `hidden`/`deleted` rows, which carry a fresh
  ...
  soft (`status='deleted'`) so the tombstone can propagate; physical purge is a
  later retention concern.
...
everyone. Finer trust tuning (scaling the threshold to consumer count, review
queues, unhide flows) is deferred until a second consumer actually exists.
